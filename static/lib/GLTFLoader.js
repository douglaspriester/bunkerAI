...
// Inlined from BufferGeometryUtils to avoid external dependency
function toTrianglesDrawMode( geometry, drawMode ) {
	if ( drawMode === 0 ) { // TrianglesDrawMode
		return geometry;
	}
	...
	const newIndices = [];
	if ( drawMode === 1 ) { // TriangleStripDrawMode
	...
	} else if ( drawMode === 2 ) { // TriangleFanDrawMode
		for ( let i = 1; i <= numberOfTriangles; i ++ ) {
			newIndices.push( index.getX( 0 ), index.getX( i ), index.getX( i + 1 ) );
		}
	}
	if ( newIndices.length === 0 ) return geometry;
